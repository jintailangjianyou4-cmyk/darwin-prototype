...
void darwin_observer_boot(darwin_observer_t *o) {
    ...
    darwin_observer_log(o, "NOTICE", "boot: kernel handoff complete");
    darwin_observer_log(o, "INFO", "boot: Mach zones and IPC initialized");
    darwin_observer_log(o, "INFO", "boot: BSD VM, credentials, and VFS initialized");
    darwin_observer_log(o, "INFO", "boot: platform expert matched root device");
    darwin_observer_log(o, "INFO", "boot: IOKit registry published");
    darwin_observer_log(o, "INFO", "boot: filesystem root mounted read-write");
    darwin_observer_log(o, "INFO", "boot: launchd bootstrap namespace created");
    darwin_observer_log(o, "INFO", "boot: system services loaded from launchd plist set");
    darwin_observer_log(o, "NOTICE", "boot: Darwin userspace reached multi-user state");
}

void darwin_observer_shutdown(darwin_observer_t *o) {
    o->shutdown_requested = 1;
    darwin_observer_log(o, "NOTICE", "shutdown: received halt request");
    darwin_observer_log(o, "INFO", "shutdown: stopping user services");
    for (int i = o->service_count - 1; i >= 0; --i) {
        if (strcmp(o->services[i].state, "running") == 0 && o->services[i].pid > 3) {
            char message[160];
            snprintf(message, sizeof(message), "launchd: stopping %s", o->services[i].label);
            darwin_observer_log(o, "NOTICE", message);
            snprintf(o->services[i].state, sizeof(o->services[i].state), "stopped");
            o->services[i].pid = -1;
        }
    }
    darwin_observer_log(o, "INFO", "shutdown: unmounting virtual filesystems");
    darwin_observer_log(o, "INFO", "shutdown: draining Mach ports");
...
int darwin_spawn(darwin_observer_t *o, const char *name) {
    ...
    char message[128]; snprintf(message, sizeof(message), "bsd: process %d (%s) created", pid, name);
    darwin_observer_log(o, "INFO", message);
    return pid;
}

int darwin_kill(darwin_observer_t *o, int pid) {
    for (int i = 0; i < o->process_count; ++i) if (o->processes[i].pid == pid && pid > 3) {
        char message[96]; snprintf(message, sizeof(message), "bsd: process %d terminated", pid);
...
int darwin_send_message(darwin_observer_t *o, int port, const char *message) {
    ...
    darwin_port_t *p = &o->ports[o->port_count++]; p->name = port; p->owner_pid = 1;
    snprintf(p->rights, sizeof(p->rights), "send+receive");
    char log_message[200]; snprintf(log_message, sizeof(log_message), "mach_msg: port=%d payload=%s", port, message);
...
int darwin_service_print(const darwin_observer_t *o, const char *label) {
    for (int i = 0; i < o->service_count; ++i) if (!strcmp(o->services[i].label, label)) {
...
int darwin_service_set_state(darwin_observer_t *o, const char *label, int running) {
    for (int i = 0; i < o->service_count; ++i) if (!strcmp(o->services[i].label, label)) {
        ...
        if (running) s->runs++;
        char message[160]; snprintf(message, sizeof(message), "launchd %s service %s", running ? "started" : "stopped", label);
        darwin_observer_log(o, "INFO", message);
        return 0;
    }
    return -1;
